...
- BM25 index is built entirely inside `RetrievalFactory._build_bm25_from_vectorstore()` — the pipeline never imports concrete retrieval classes.
...
## VectorStoreInterface — Required Methods

All vector store implementations must satisfy this contract:

| Method | Signature | Purpose |
|---|---|---|
| `upsert` | `(chunks, embeddings) → None` | Write chunks + vectors |
| `search` | `(query_emb, top_k, filters) → List[Dict]` | ANN similarity search |
| `delete_by_doc_id` | `(doc_id) → None` | Remove all chunks for a document |
| `get_all_documents` | `() → (corpus, ids)` | Full corpus for BM25 index |
| `get_all_documents_with_metadata` | `() → (corpus, ids, metadata)` | Corpus + metadata for filtered BM25 |
| `count` | `() → int` | Total vector count (used for empty-store checks) |
| `update_document_metadata` | `(doc_id, updates) → int` | Merge metadata updates; returns chunks updated |
